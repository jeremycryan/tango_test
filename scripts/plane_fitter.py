...
class PlaneFitter(object):
    def __init__(self):
        rospy.init_node('floor_finder') #Initialize Node for Code
        ...
        self.pub = rospy.Publisher('/plane_finder', PointCloud, queue_size=10) #Be ready to publish pointclouds to plane_finder, cloud_transformed, and plane_lines
        ...
        self.resolutionfactor = 10 #Factor by which to lower the resolution of the point cloud (i.e. if it is 10, we only look at every 10th point of the pointcloud.)
        ...
        while not rospy.is_shutdown(): #Start main while loop
            if not self.actualP is None and not self.position is None: #Wait until phone has found a pointcloud and a phone position
                ...
                if actualPCopy is None: #If something went wrong with the transform (if the transforms haven't been set yet or some timing issues happen)
                    ...
                    if rospy.Time.now()-self.last_sound_time > rospy.Duration(1) and self.walldist is not 0: #If it's been longer than 2 seconds since last sound, and the wall distance is greater than half a meter
                        ...
                        if abs(self.walldist) < self.altsounddist:
                            ...
                            self.synth = ac.synth(freq) #create synth for sound
                            self.synth = ac.delay(self.synth, 3, 250, .5) #add delay to the synth
                            ac.player(self.synth, seconds=.75, volume = 1) #play the sound.
